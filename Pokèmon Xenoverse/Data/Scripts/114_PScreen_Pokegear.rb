...
# OPTIONS - If you change these, you should also change update_command below.
    ...
    @cmdPhone=-1
    @cmdObiettivi=-1
    commands[@cmdMap=commands.length]=_INTL("Mappa")
    commands[@cmdPhone=commands.length]=_INTL("Phone") if $PokemonGlobal.phoneNumbers &&
                                                          $PokemonGlobal.phoneNumbers.length>0
    commands[@cmdObiettivi=commands.length]=_INTL("Obiettivi")
  ...
  def update_command
    ...
    if Input.trigger?(Input::C)
      ...
      if @cmdObiettivi>=0 && @sprites["command_window"].index==@cmdObiettivi
        pbPlayDecisionSE()
        pbFadeOutIn(99999) {
