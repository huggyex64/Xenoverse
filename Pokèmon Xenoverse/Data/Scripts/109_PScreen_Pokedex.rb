...
class Scene_PokedexMenu
  ...
  def main
    ...
    @sprites["headings"]=Window_AdvancedTextPokemon.newWithSize(
       _INTL("<c3=F8F8F8,C02028>VISTI<r>CATTURATI</c3>"),286,104,208,64,@viewport)
...
class Window_Pokedex < Window_DrawableCommand
  def initialize(x,y,width,height)
    @pokeballOwned=AnimatedBitmap.new("Graphics/Pictures/Dex/pokedexOwned")
    @pokeballSeen=AnimatedBitmap.new("Graphics/Pictures/pokedexSeen")
    @commands=[]
    super(x,y,width,height)
    self.windowskin=nil
    self.baseColor=Color.new(250,250,250)
    self.shadowColor=Color.new(103,78,114)
  end
...
class PokemonPokedexScene
  def pbUpdate
    ...
    if $PokemonGlobal.pokedexDex==0
      ...
      @sprites["animbg"].oy+=1.5
      @sprites["animbg"].ox-=1.5
    end
    #@sprites["icon"].setSpeed(1)
    #@sprites["icon"].play    
    #@sprites["icon"].update
  ...
  def setIconBitmap(species)
    @sprites["icon"].setDexSpeciesBitmap(species)
    ...
    if @sprites["icon"].bitmap
      ...
      @sprites["icon"].oy=@sprites["icon"].bitmap.height
    end
  end
...
# National Dex at the end.
  ...
  def pbStartScene
    ...
    if $PokemonGlobal.pokedexDex=0
      ...
      @sprites["species"].baseColor=Color.new(240,240,240)
      ...
      @sprites["seen"].baseColor=Color.new(240,240,240)
      ...
      @sprites["owned"].baseColor=Color.new(240,240,240)
      ...
      @sprites["owned"].baseColor=Color.new(240,240,240)
      @sprites["owned"].shadowColor=Color.new(168,184,184)
      addBackgroundPlane(@sprites,"searchbg",_INTL("xpokedexSearchbg"),@viewport)
      @sprites["searchbg"].visible=false
      @searchResults=false
      @sprites["background"]=Sprite.new(@viewport)
...
=end
  ...
  def pbRefreshDexList(index=0)
    ...
    #@sprites["icon"].setSpeciesBitmap(iconspecies)
    #if @sprites["icon"].bitmap
    #  @sprites["icon"].ox=@sprites["icon"].bitmap.width/2
    #  @sprites["icon"].oy=@sprites["icon"].bitmap.height/2
    #end
    #setIconBitmap(pbLoadPokemonBitmapSpecies(pokemon,iconspecies))#(iconspecies,false))
    if iconspecies>0
  ...
  def pbPokedex
    pbActivateWindow(@sprites,"pokedex"){
       loop do
         ...
         if Input.trigger?(Input::B) && $PokemonGlobal.pokedexDex==0
           pbPlayCancelSE()
           if @searchResults
             pbCloseSearch
           else
         ...
         elsif Input.trigger?(Input::B) && $PokemonGlobal.pokedexDex==1
           pbPlayCancelSE()
           $PokemonGlobal.pokedexDex=0
           if @searchResults
             pbCloseSearch
           else
             break
           end
         elsif Input.trigger?(Input::A) && $PokemonGlobal.pokedexDex==0
...
class PokemonPokedex
  ...
  def pbDexEntry(pokemon)
    @scene.pbStartDexEntryScene(pokemon)
