...
class Scene_Intro
  ...
  def main
    ...
    # used for super awesome feature
    @viewnew = Viewport.new(0,0,Graphics.width,Graphics.height)
    @viewnew.z = 999999
    @img = Sprite.new(@viewnew)
    #@img.onlineBitmap("http://i.imgur.com/CuClkGU.png") if (Time.now.mon == 4 && Time.now.day == 1)
  ...
  def closeTitle
    # Play Pokemon cry
    pbSEPlay("DRAGALISK.wav",70,100) if @cry && SCREENSTYLE!=6
    # Fade out
    pbBGMStop(1.0)
...
# Utilities
#===============================================================================
class SliderTitle_Sprite < Sprite
  ...
  def initialize(pokemon)
    ...
    viewport.z = 99998
    super(viewport)
    self.bitmap = Bitmap.new("Graphics/Titles/" + pokemon)
  end
end
#===============================================================================
# Xenoverse Styled title screen
#===============================================================================
class XenoverseStyle
  include EAM_Sprite
  
  def initialize
    #variables & misc handling
    @skip = false
    @speed = 16
    @opacity = 17
    @disposed = false
    @red = Color.new(219,62,48)
    @blue = Color.new(51,61,119)
    @starglow = false
    @increase = 0.5
    @lincrease = 0.1
    
    #start intro music
    pbBGMPlay("intro.mp3",80)
    ...
    @sprites = {}
		...
		@sprites["starbg"]=Sprite.new(@viewport)
    @sprites["starbg"].bitmap=pbBitmap("Graphics/Titles/Intro/background")
    @sprites["starbg"].opacity=0
    @sprites["starbg"].z=-2
		
    @sprites["bg"]=Sprite.new(@viewport)
    @sprites["bg"].bitmap=pbBitmap("Graphics/Titles/bg")
    @sprites["bg"].visible = false
    @sprites["bg"].z=-1
		@sprites["logo"] = Sprite.new(@viewport)
		@sprites["logo"].bitmap = pbBitmap("Graphics/Titles/xenologo")
		@sprites["logo"].visible = false
		@sprites["loglow"] = Sprite.new(@viewport)
		@sprites["loglow"].bitmap = pbBitmap("Graphics/Titles/xenologo2")
		@sprites["loglow"].opacity = 0
		
		@sprites["luxflonstart"] = Sprite.new(@viewport)
		@sprites["luxflonstart"].bitmap = pbBitmap("Graphics/Titles/luxflonstart")
		@sprites["luxflonstart"].opacity = 0
		
		@sprites["dragaliskstart"] = Sprite.new(@viewport)
		@sprites["dragaliskstart"].bitmap = pbBitmap("Graphics/Titles/dragaliskstart")
		@sprites["dragaliskstart"].opacity = 0
		
		@sprites["luxflon"] = Sprite.new(@viewport)
		@sprites["luxflon"].bitmap = pbBitmap("Graphics/Titles/Intro/luxflon")
		@sprites["luxflon"].opacity = 0
		
		@sprites["dragalisk"] = Sprite.new(@viewport)
		@sprites["dragalisk"].bitmap = pbBitmap("Graphics/Titles/Intro/dragalisk")
		@sprites["dragalisk"].opacity = 0
		
		@sprites["astro"] = Sprite.new(@viewport)
		@sprites["astro"].bitmap = pbBitmap("Graphics/Titles/Intro/astro")
		@sprites["astro"].opacity = 0
		
		@sprites["bar"]=Sprite.new(@viewport)
    @sprites["bar"].bitmap=Bitmap.new(Graphics.width,25)
    @sprites["bar"].y=304
    @sprites["bar"].bitmap.fill_rect(0,0,Graphics.width,25, Color.new(0,0,0))
		@sprites["bar"].opacity = 128
    @sprites["bar"].visible = false
		
		for i in 0..1
      @sprites["star#{i}"]=Sprite.new(@viewport)
      @sprites["star#{i}"].bitmap=pbBitmap("Graphics/Titles/star#{i}")
      @sprites["star#{i}"].opacity= (i>0) ? 0 : 0
    end
		
    @sprites["start"]=Sprite.new(@viewport)
    @sprites["start"].bitmap=pbBitmap("Graphics/Titles/titlestart")
    @sprites["start"].x=Graphics.width/2 - @sprites["start"].bitmap.width/4
    @sprites["start"].y=Graphics.height/2 + 117
    @sprites["start"].zoom_x=0.5
    @sprites["start"].zoom_y=0.5
    @sprites["start"].opacity=0
    
  end
  
  def intro
    wait(121,false) if !@skip
		r=0
    if !@skip
      30.times do
        Graphics.update
        Input.update
        r+=255/29
        @sprites["starbg"].opacity = r
        @skip = true if Input.trigger?(Input::C)
      end
    end
		r = 0
		wait(120-50,false) if !@skip
    if !@skip
      20.times do
        Graphics.update
        Input.update
        r+=255/19
        @sprites["luxflon"].opacity = r
        @skip = true if Input.trigger?(Input::C)
      end
    end
		wait(60,false) if !@skip
		r = 255
    if !@skip
      20.times do
        Graphics.update
    ...
    10.times do
      ...
      Input.update 
      r+=255/9
      @viewport.color = Color.new(255,255,255,r)
    end
		@sprites["astro"].opacity = 0
    @sprites["dragalisk"].visible = false
    @sprites["astro"].visible = false
    @sprites["luxflon"].visible = false
		@sprites["logo"].visible = true
		@sprites["bg"].visible = true
		@sprites["starbg"].visible = false
  ...
  def update
    ...
    @frame+=1
		@glowFrame+=1
		if @glowFrame<55
			@sprites["loglow"].opacity+=255/54
		else
			@sprites["loglow"].opacity-=255/54
		end
		
		if @glowFrame > 50 && @glowFrame < 90
			@sprites["luxflonstart"].opacity+=180/39
			@sprites["dragaliskstart"].opacity-=180/39
		elsif @glowFrame > 200 && @glowFrame < 240
			@sprites["luxflonstart"].opacity-=180/39
			@sprites["dragaliskstart"].opacity+=180/39
		else
			@sprites["luxflonstart"].opacity-=180/39
...
class GenOneStyle  
  def initialize
    ...
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_ONE_BGM
...
class GenTwoStyle
  def initialize
    # sound file for playing the title screen BGM
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_TWO_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(bgm).name
    @mp3 = (File.extname(str)==".ogg") ? true : false
    @skip = false
    # speed of the effect movement
...
class GenThreeStyle
  ...
  def initialize
    # sound file for playing the title screen BGM
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_THREE_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(bgm).name
    @mp3 = (File.extname(str)==".ogg") ? true : false
    @skip = false
    # speed of the effect movement
    ...
    @sprites["bg2"].bitmap = pbBitmap("Graphics/Titles/gen_3_bg_ds1") if DS_STYLE || @new
  ...
  def intro
    ...
    if DS_STYLE || @new
      ...
      @sprites["bg2"].bitmap = pbBitmap("Graphics/Titles/gen_3_bg_ds2") if DS_STYLE
      if !DS_STYLE
       @sprites["logo1"].y-=64
       @sprites["logo2"].y-=64
...
class GenFourStyle
  def initialize
    # sound file for playing the title screen BGM
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_FOUR_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(bgm).name
    @mp3 = (File.extname(str)==".ogg") ? true : false
    @skip = false
...
class GenFiveStyle
  ...
  def initialize
    ...
    # sound file for playing the title screen BGM
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_FIVE_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(bgm).name
    @mp3 = (File.extname(str)==".ogg") ? true : false
    @skip = false
    # speed of the silhouette animation
...
class GenSixStyle
  def initialize
    ...
    # sound file for playing the title screen BGM
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_SIX_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(bgm).name
    @mp3 = (File.extname(str)==".ogg") ? true : false
    @skip = false
    @disposed = false
...
class GenSevenStyle
  def initialize
    # sound file for playing the title screen BGM
    @bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_SEVEN_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(@bgm).name
    @skip = false
    @disposed = false
...
class GenCustomStyle
  def initialize
    ...
    @showPoke = true
    # sound file for playing the title screen BGM
    bgm = (Time.now.mon == 4 && Time.now.day == 1 && FileTest.exist?("Audio/BGM/time.ogg")) ? "time.ogg" : GEN_CUSTOM_BGM
    str = "Audio/BGM/"+pbResolveAudioFile(bgm).name
    @mp3 = (File.extname(str)==".ogg") ? true : false
    @skip = false
    @disposed = false
