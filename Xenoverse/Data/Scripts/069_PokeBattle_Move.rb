class PokeBattle_Move
		...
		if type>=0 
			...
			if attacker.hasWorkingAbility(:NORMALIZE)
				...
				if attacker.hasWorkingAbility(:AERILATE) && hasConst?(PBTypes,:FLYING)
					...
					@powerboost=true
				elsif attacker.hasWorkingAbility(:EQUALIZE) && hasConst?(PBTypes,:SUONO)
					type=getConst(PBTypes,:SUONO)
					@powerboost=true
				elsif attacker.hasWorkingAbility(:SCALATE) && hasConst?(PBTypes,:DRAGON)
  ...
  def ignoresSubstitute?(attacker)
	...
	def pbTypeModifier(type,attacker,opponent)
		...
		if (isConst?(otype1,PBTypes,:FIRE)||isConst?(otype2,PBTypes,:FIRE)) && opponent.effects[PBEffects::BurnUp]
			if otype1==otype2 #is only fire type
				otype1=getConst(PBTypes,:NORMAL) || 0
				otype2=otype1
			elsif isConst?(otype1,PBTypes,:FIRE) #type 1 is fire, whilst type 2 is not
				otype1=getConst(PBTypes,:NORMAL) || 0
			elsif isConst?(otype2,PBTypes,:FIRE) #type 2 is fire, so it becomes same as type1
				otype2=otype1
			end
		end
		...
		if opponent.effects[PBEffects::Ingrain] ||
			...
			mod3=2 if isConst?(otype2,PBTypes,:FLYING) && isConst?(atype,PBTypes,:GROUND)
		end
		...
		if @function==0x16D #&& !attacker.effects[PBEffects::Electrify] # Freeze-Dry
...
=end
	...
	def pbAccuracyCheck(attacker,opponent)
		return false if @function==0x6E && opponent.boss
	...
	def pbCalcDamage(attacker,opponent,options=0)
		...
		if (attacker.hasWorkingAbility(:AERILATE) ||
				...
				attacker.hasWorkingAbility(:SCALATE)||
				attacker.hasWorkingAbility(:EQUALIZE)||
		...
		if attacker.pbHasType?(type) && (options&IGNOREPKMNTYPES)==0
			if attacker.hasWorkingAbility(:ADAPTABILITY) ||	
				attacker.hasWorkingAbility(:JUNGLEMIRAGE) ||
				attacker.hasWorkingAbility(:VOICETUNING) ||
				attacker.hasWorkingAbility(:DRAGONARMOR)
		...
		if opponent.damagestate.typemod>8 && (options&IGNOREPKMNTYPES)==0
			if (opponent.hasWorkingItem(:CHOPLEBERRY) && isConst?(type,PBTypes,:FIGHTING)) ||
				...
				@battle.pbDisplayPaused(_INTL("{1} consumed its {2} and resisted the damage!",opponent.name,PBItems.getName(opponent.item)))
				finaldamagemult=(finaldamagemult*0.5).round
	...
	def pbReduceHPDamage(damage,attacker,opponent)
		...
		if opponent.effects[PBEffects::Substitute]>0 && (!attacker || attacker.index!=opponent.index)
			...
			echoln opponent.pokemon.name
			echoln opponent.pokemon.busted
			if opponent.hasWorkingAbility(:DISGUISE) && isConst?(opponent.species,PBSpecies,:MIMIKYU) && !attacker.hasMoldBreaker
				if !opponent.pokemon.busted
					
					@battle.pbDisplay(_INTL("Its disguise served it as a decoy!"))
					@battle.scene.pbDisplayEffect(opponent)
					@battle.scene.pbDamageAnimation(opponent,0)
					opponent.pbBust
					damage=0
				end
			...
			opponent.hp-=damage
			effectiveness=0
			if opponent.damagestate.typemod<8#<4
				effectiveness=1   # "Not very effective"
			elsif opponent.damagestate.typemod>8#>4
	...
	def pbEffectMessages(attacker,opponent,ignoretype=false)
		...
		if !pbIsMultiHit
			...
			elsif opponent.damagestate.typemod>=1 && opponent.damagestate.typemod<8#<4
	...
	def pbDisplayUseMessage(attacker)
		...
		@battle.pbDisplayBrief(_INTL("{1} usa\r\n{2}!",attacker.pbThis,name))
		return 0
	end
	...
	def pbOnDamageLost(damage,attacker,opponent)
		...
		if pbIsPhysical?(type)
			...
			opponent.effects[PBEffects::ShellTrap]=attacker.index
		end
		if pbIsSpecial?(type)
