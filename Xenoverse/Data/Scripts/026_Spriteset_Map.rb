class ReflectedSprite
  ...
  def update
    ...
    while i<@rsprite.src_rect.height+Game_Map::TILEHEIGHT
      ...
      if @event.map.terrain_tag(@event.x,nextY)!=PBTerrain::StillWater && @event.map.terrain_tag(@event.x,nextY)!=PBTerrain::Reflect
    ...
    if @sprite
      ...
      stillreflect = false
      stillreflect = true if @event.map.terrain_tag(@event.x,@event.y)==PBTerrain::Reflect
      if @event.map.terrain_tag(@event.x,@event.y)==PBTerrain::StillWater && @event.map.terrain_tag(@event.x,nextY)!=PBTerrain::Reflect
      ...
      if frame==1
        @sprite.zoom_x*=stillreflect ? 1 : 1.05    
      elsif frame==2
        @sprite.zoom_x*=stillreflect ? 1 : 1.1    
      elsif frame==3
        @sprite.zoom_x*=stillreflect ? 1 : 1.05   
      end
      @sprite.mirror=true
