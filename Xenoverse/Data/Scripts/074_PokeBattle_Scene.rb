...
class PokemonDataBox < SpriteWrapper
  ...
  def refresh
    ...
    if isConst?(@battler.species,PBSpecies,:SPARKOWL)
      imagepos.push(["Graphics/Pictures/delta.png",@spritebaseX+8,34,0,0,-1,-1])
    end
...
class PokeBattle_Scene
  ...
  def pbDisplayConfirmMessage(msg)
    return pbShowCommands(msg,[_INTL("Sì"),_INTL("No")],1)==0
  end
  ...
  def pbStartBattle(battle)
    ...
    if @battle.opponent
      ...
      if isConst?(@battle.party2[0].species,PBSpecies,:ELEKIDX) && @battle.battlescene
        pbCommonAnimation("ELEKIDX",@battle.battlers[1],nil)
      end
      if @battle.party2.length==2
        if @battle.party2[1].isShiny? && @battle.battlescene
          pbCommonAnimation("Shiny",@battle.battlers[3],nil)
        end
         if isConst?(@battle.party2[1].species,PBSpecies,:ELEKIDX) && @battle.battlescene
          pbCommonAnimation("ELEKIDX",@battle.battlers[3],nil)
        end
      end
    end
  end
