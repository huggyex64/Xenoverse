...
class PokemonEggHatchScene
  def pbStartScene(pokemon)
    ...
    pbFadeInAndShow(@sprites)
		@sprites["test"] = coloredRect(@sprites["pokemon"].x,@sprites["pokemon"].y,@sprites["pokemon"].bitmap.width,@sprites["pokemon"].bitmap.height,@viewport)
		@sprites["test"].z = 200
		printSpriteInfo(@sprites["test"])
  end
  ...
  def pbMain
    ...
    @sprites["pokemon"].oy = pbGetSpriteBase(@sprites["pokemon"].bitmap)
		@sprites["pokemon"].x=Graphics.width/2-@sprites["pokemon"].bitmap.width/2
    #@sprites["pokemon"].y=48+Graphics.height/2-@sprites["pokemon"].bitmap.height/2
    @sprites["pokemon"].y = 254
    for i in 1..(255/fadeSpeed)
