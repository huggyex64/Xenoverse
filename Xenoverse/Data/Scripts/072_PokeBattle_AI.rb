...
module PBTrainerAI
  ...
  def PBTrainerAI.ultraSkill; 127; end
end
...
##### Alter score depending on the move's function code ########################
    ...
    when 0x27, 0x28
      if skill == PBTrainerAI.ultraSkill && (attacker.stages[PBStats::ATTACK] < 6 || attacker.stages[PBStats::SPATK] < 6)
        if !pbCanOneshot(attacker, opponent, skill)
          attacker.stages[PBStats::ATTACK] += 1
    ...
    when 0x11F
      if skill == 127
        if @field.effects[PBEffects::TrickRoom]>0
          score-=1000
        elsif @field.effects[PBEffects::TrickRoom]==0
          score+=200
        end
      end
    when 0x120
...
################################################################################
  def pbTypeModifier(type,attacker,opp)
    ...
    if opponent.respond_to?(:effects=) && (isConst?(otype2,PBTypes,:FLYING) && opponent.effects[PBEffects::Roost])
      otype2=otype1
    end
    mod1=PBTypes.getEffectiveness(atype,otype1)
    mod2=(otype1==otype2) ? 2 : PBTypes.getEffectiveness(atype,otype2)
    if attacker.hasWorkingAbility(:SCRAPPY) || (opponent.respond_to?(:effects=) &&
    ...
    if opponent.respond_to?(:effects=) && (opponent.effects[PBEffects::Ingrain] ||
       opponent.effects[PBEffects::SmackDown] ||
    ...
    if opponent.respond_to?(:effects=) && opponent.effects[PBEffects::MiracleEye]
  ...
  def pbRoughDamage(move,attacker,opponent,skill,basedamage)
    ...
    # Iron Fist
    if skill>=PBTrainerAI.mediumSkill && attacker.respond_to?(:hasWorkingAbility=)
    ...
    # Flare Boost
    if skill>=PBTrainerAI.highSkill && attacker.respond_to?(:hasWorkingAbility=)
    ...
    # Heatproof
    if skill>=PBTrainerAI.bestSkill && opponent.respond_to?(:hasWorkingAbility=)
    ...
    # Sheer Force
    if skill>=PBTrainerAI.highSkill && opponent.respond_to?(:hasWorkingAbility=)
    ...
    if (attacker.hasWorkingItem(:FISTPLATE) && isConst?(type,PBTypes,:FIGHTING)) ||
       ...
       (attacker.hasWorkingItem(:SOUNDPLATE) && isConst?(type,PBTypes,:SUONO))
      basedamage=(basedamage*1.2).round
    end
    ...
    # Sandstorm weather
    if skill>=PBTrainerAI.highSkill && opponent.respond_to?(:pbHasType=)
    ...
    # Marvel Scale
    if skill>=PBTrainerAI.bestSkill && opponent.respond_to?(:hasWorkingAbility=) && opponent.respond_to?(:status=)
    ...
    if skill>=PBTrainerAI.highSkill && opponent.respond_to?(:hasWorkingItem=)
    ...
    # Reflect
    if skill>=PBTrainerAI.highSkill && opponent.respond_to?(:pbOwnSide=)
    ...
    if typemod>4 && skill>=PBTrainerAI.highSkill && opponent.respond_to?(:hasWorkingItem=)
    ...
    if !wildbattle && maxscore>100
      ...
      if stdev>=65 && pbAIRandom(10)!=0
        ...
        if preferredMoves.length>0 && (skill < PBTrainerAI.ultraSkill || maxscore >=100) #to adjust
          ...
          finalChoice = pbHighestScore(scores)
          pbRegisterMove(index,finalChoice,false)
          target=targets[finalChoice] if targets
        end
      end
    end
  ...
  def pbHighestDamageMove(attacker, opponent, skill)
    ...
    for move in attacker.moves
      ...
      if basedamage > highestDamage
        highestDmgMove = move
        highestDamage = basedamage
      end
    end
    return highestDmgMove
  end

  def pbAttacker(index)
    return @battlers[index]
  end

  def pbOpponent(index)
    return pbAttacker(index).pbOppositeOpposing
  end
