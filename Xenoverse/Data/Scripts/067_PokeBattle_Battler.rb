class PokeBattle_Battler
	...
	def status=(value)
		return if @effects[PBEffects::Substitute]>0
	...
	def pbThis(lowercase=false)
		if @battle.pbIsOpposing?(@index)
			if @battle.opponent
				return lowercase ? _INTL("{1} nemico",@name) : _INTL("{1} nemico",@name)
			else
				return lowercase ? _INTL("{1} selvatico",@name) : _INTL("{1} selvatico",@name)
			end
	...
	def pbHasType?(type)
		return false if @effects[PBEffects::BurnUp]
		if type.is_a?(Symbol) || type.is_a?(String)
			...
			return (self.type1==type || self.type2==type || @effects[PBEffects::Type3]==type)
		end
	end
	...
	def pbCheckForm
		...
		if isConst?(self.species, PBSpecies,:RAPIDASHX) && !self.isFainted?
			...
			if transformed
				pbUpdate(true)
				if(self.form==1)
					@battle.pbDisplay(_INTL("{1} è su tutte le furie!",pbThis))
				end
			end
		end
		# Keldeo
	...
	def pbResetForm
		if !@effects[PBEffects::Transform]
			if isConst?(self.species,PBSpecies,:CASTFORM) ||
				...
				isConst?(self.species,PBSpecies,:RAPIDASHX) ||
				isConst?(self.species,PBSpecies,:CHIMAOOZE)
				self.form=0
			end
		end
		pbUpdate(true)
	...
	def pbAbilitiesOnSwitchIn(onactive)
		...
		# Dreadspace message
		if self.hasWorkingAbility(:DREADSPACE) && onactive
			@battle.scene.pbDisplay(_INTL("{1} emanates a freezing aura!",pbThis))
			PBDebug.log("[#{pbThis}: has Dreadspace]")
		end
		# Trace
		...
		if self.hasWorkingAbility(:HOLYGUARD) && onactive
			...
			target = @battle.pbOwnedByPlayer?(@index) ? @index + 1 : @index-1
	...
	def pbEffectsOnDealingDamage(move,user,target,damage)
		...
		if damage>0 && move.isContactMove? && !user.hasWorkingItem(:PROTECTIVEPADS) && !user.hasWorkingAbility?(:LONGREACH)
			if !target.damagestate.substitute
				...
				movex=@battle.choices[target.index][2]
				if moves.is_a?(PokeBattle_Move)
					if movex.function==0x176 && # Shell Trap
				...
				# Innards Out
				if target.hasWorkingAbility(:INNARDSOUT,true) && target.fainted? && 
				...
				if target.hasWorkingAbility(:STATIC,true) &&
					...
					user.pbParalyze(target)
					@battle.scene.pbDisplayEffect(target)
					@battle.scene.pbDisplay(_INTL("{1}'s {2} paralyzed {3}!  It may be unable to move!",
							target.pbThis,PBAbilities.getName(target.ability),user.pbThis(true)))
				end
			...
			if user.hasWorkingAbility(:POISONTOUCH,true) &&
				...
				@battle.scene.pbDisplayEffect(user)
				@battle.scene.pbDisplay(_INTL("{1}'s {2} poisoned {3}!",user.pbThis,
		...
		if damage>0
			...
			if target.hasWorkingItem(:ABSORBBULB) && isConst?(movetype,PBTypes,:WATER)
				...
				target.pbIncreaseStatWithCause(PBStats::SPATK,1,target,PBItems.getName(target.item))
			...
			if target.hasWorkingItem(:CELLBATTERY) && isConst?(movetype,PBTypes,:ELECTRIC)
				...
				target.pbIncreaseStatWithCause(PBStats::ATTACK,1,target,PBItems.getName(target.item))
				#if target.pbCanIncreaseStatStage?(PBStats::ATTACK)
				#	target.pbIncreaseStatBasic(PBStats::ATTACK,1)
				#	@battle.pbCommonAnimation("StatUp",target,nil)
				#	@battle.pbDisplay(_INTL("{1}'s {2} raised its Attack!",
			...
			if target.hasWorkingItem(:LUMINOUSMOSS) && isConst?(movetype,PBTypes,:WATER)
				...
				target.pbIncreaseStatWithCause(PBStats::SPDEF,1,target,PBItems.getName(target.item))
	...
	def pbConfusionBerry(symbol,flavor,message1,message2)
		if isConst?(self.item,PBItems,symbol) && self.hp<=(self.totalhp/4).floor
...
=end
  ...
  alias isFainted? fainted?
    ...
    if isConst?(berry,PBItems,:ORANBERRY)
      ...
      consumed=pbConfusionBerry(:FIGYBERRY,0,
      ...
      consumed=pbConfusionBerry(:WIKIBERRY,3,
         _INTL("{1}'s {2} restored health!",pbThis,berryname),
         _INTL("For {1}, the {2} was too dry!",pbThis(true),berryname))
    elsif isConst?(berry,PBItems,:MAGOBERRY)
      consumed=pbConfusionBerry(:MAGOBERRY,2,
         _INTL("{1}'s {2} restored health!",pbThis,berryname),
         _INTL("For {1}, the {2} was too sweet!",pbThis(true),berryname))
    elsif isConst?(berry,PBItems,:AGUAVBERRY)
      consumed=pbConfusionBerry(:AGUAVBERRY,4,
         _INTL("{1}'s {2} restored health!",pbThis,berryname),
         _INTL("For {1}, the {2} was too bitter!",pbThis(true),berryname))
    elsif isConst?(berry,PBItems,:IAPAPABERRY)
      consumed=pbConfusionBerry(:IAPAPABERRY,1,
         _INTL("{1}'s {2} restored health!",pbThis,berryname),
         _INTL("For {1}, the {2} was too sour!",pbThis(true),berryname))
    elsif isConst?(berry,PBItems,:LIECHIBERRY)
      consumed=pbStatIncreasingBerry(PBStats::ATTACK,berryname,_INTL("Using its {1}, the Attack of {2} rose!",berryname,pbThis(true)))
    elsif isConst?(berry,PBItems,:GANLONBERRY) ||
          isConst?(berry,PBItems,:KEEBERRY)
      consumed=pbStatIncreasingBerry(PBStats::DEFENSE,berryname,_INTL("Using its {1}, the Defense of {2} rose!",berryname,pbThis(true)))
    elsif isConst?(berry,PBItems,:SALACBERRY)
      consumed=pbStatIncreasingBerry(PBStats::SPEED,berryname,_INTL("Using its {1}, the Speed of {2} rose!",berryname,pbThis(true)))
    elsif isConst?(berry,PBItems,:PETAYABERRY)
      consumed=pbStatIncreasingBerry(PBStats::SPATK,berryname,_INTL("Using its {1}, the Special Attack of {2} rose!",berryname,pbThis(true)))
    elsif isConst?(berry,PBItems,:APICOTBERRY) ||
          isConst?(berry,PBItems,:MARANGABERRY)
      consumed=pbStatIncreasingBerry(PBStats::SPDEF,berryname,_INTL("Using its {1}, the Special Defense of {2} rose!",berryname,pbThis(true)))
		...
		if thismove.canSnatch?
			for i in priority
				if i.effects[PBEffects::Snatch]
					...
					#lower PP even more if target has dreadspace
					if target.hasWorkingAbility(:DREADSPACE) && userchoice>=0
						pressuremove=user.moves[userchoice]
	...
	def pbChangeTarget(thismove,userandtarget,targets)
		...
		if target.hasWorkingAbility(:DREADSPACE)
			PBDebug.log("[#{target.pbThis}'s Dreadspace triggered (pbChangeTarget)]")
			pbReducePP(thismove) # Reduce PP
			pbReducePP(thismove) # Reduce PP
			pbReducePP(thismove) # Reduce PP
		end 
		...
		if thismove.canSnatch?
			for i in priority
				if i.effects[PBEffects::Snatch]
					...
					if target.hasWorkingAbility(:DREADSPACE) && userchoice>=0
						PBDebug.log("[#{target.pbThis}'s Dreadspace triggered (part of Snatch)]")
						pressuremove=user.moves[userchoice]
						pbSetPP(pressuremove,pressuremove.pp-2) if pressuremove.pp>0
					end
				end
		...
		if accuracy
			...
			if miss || !thismove.pbAccuracyCheck(user,target) # Includes Counter/Mirror Coat
				...
				if user.hasWorkingItem(:BLUNDERPOLICY)
					...
					user.pbIncreaseStatWithCause(PBStats::SPEED,2,user,PBItems.getName(user.item))
	...
	def pbUseMove(choice,specialusage=false)
		...
		if user.hasWorkingAbility(:PROTEAN) &&
			...
			if !user.pbHasType?(movetype)
				...
				@battle.scene.pbDisplay(_INTL("{1} transformed into the {2} type!",user.pbThis,typename))  
			end
		end
		...
		if targets.length==0
			...
			if user.hasWorkingItem(:THROATSPRAY) && thismove.isSoundBased?
				...
				user.pbIncreaseStatWithCause(PBStats::SPATK,1,target,PBItems.getName(target.item))
				user.pokemon.itemRecycle=user.item
				user.pokemon.itemInitial=0 if user.pokemon.itemInitial==user.item
				user.item=0
