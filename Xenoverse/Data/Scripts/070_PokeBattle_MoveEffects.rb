...
class PokeBattle_Move_007 < PokeBattle_Move
	...
	def pbAdditionalEffect(attacker,opponent)
		...
		if isConst?(@id,PBMoves,:THUNDERPUNCH)
			if rand(10)==5 #10% chance
				opponent.pbParalyze(attacker)
...
class PokeBattle_Move_069 < PokeBattle_Move
	def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
		...
		pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,true)
...
class PokeBattle_Move_06C < PokeBattle_Move
	def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
		return pbEffectFixedDamage((opponent.boss ? (opponent.hp/2)/opponent.hpMoltiplier : opponent.hp/2).floor,attacker,opponent,hitnum,alltargets,showanimation)
	end
end
...
class PokeBattle_Move_0D5 < PokeBattle_Move
	def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
		...
		pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
		attacker.pbRecoverHP((attacker.boss ? ((attacker.totalhp+1)/2)/attacker.hpMoltiplier : (attacker.totalhp+1)/2).floor,true)
...
class PokeBattle_Move_0DD < PokeBattle_Move
	def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
		ret=super(attacker,opponent,hitnum,alltargets,showanimation)
		if opponent.damagestate.calcdamage>0
			hpgain=(attacker.boss ? ((opponent.damagestate.hplost+1)/2)/attacker.hpMoltiplier : (opponent.damagestate.hplost+1)/2).floor
...
class PokeBattle_Move_0E5 < PokeBattle_Move
	def pbAccuracyCheck(attacker,opponent)
		return false if opponent.boss || attacker.boss
		...
		if !attacker.isFainted? && opponent.damagestate.calcdamage>0 &&
			...
			if opponent.hasWorkingAbility(:STICKYHOLD) && !attacker.hasMoldBreaker
				...
				if !attacker.hasWorkingAbility(:KLUTZ) &&
					...
					echoln "IM HERE EATING THE FUCKING BERRY"
					echoln isConst?(attacker.item,PBItems,:SITRUSBERRY)
					# Get berry's effect here
					if isConst?(attacker.item,PBItems,:LEPPABERRY)
						minid = -1
						for i in 0...attacker.pokemon.moves.length
							if attacker.pokemon.moves[i].pp<attacker.pokemon.moves[i].totalpp #has less pp than maximum
								if minid==-1 #if it's first assign it
									minid = i
								else
									minid = i if attacker.pokemon.moves[i].pp<attacker.pokemon.moves[minid].pp
								end
							end
						end
						if minid != -1
							attacker.pokemon.moves[minid].pp+=10
							attacker.pokemon.moves[minid].pp=attacker.pokemon.moves[minid].totalpp if attacker.pokemon.moves[minid].pp>attacker.pokemon.moves[minid].totalpp 
							attacker.moves[minid].pp=attacker.pokemon.moves[minid].pp
							movename=PBMoves.getName(attacker.pokemon.moves[minid].id)
							berryname = PBItems.getName(PBItems::LEPPABERRY)
							@battle.pbDisplay(_INTL("{1}'s {2} restored {3}'s PP!",attacker.pbThis,berryname,movename))
						end
					else
						attacker.pbActivateBerryEffect
					end
					attacker.item = olditem
				end
			end
		end
		return ret
	...
	def flingarray
		return {
			...
			30 => [:ABILITYURGE,:ABSORBBULB,:AMULETCOIN,:ANTIDOTE,:AWAKENING,
				...
				:LAVACOOKIE,:LEAFSTONE,:LEMONADE,:LIFEORB,:LIGHTBALL,:ALTERBALL,
	...
	def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
		...
		if !attacker.isFainted? && opponent.damagestate.calcdamage>0
			...
			if attacker.pbOwnSide.effects[PBEffects::StickyWeb]
				attacker.pbOwnSide.effects[PBEffects::StickyWeb]=false
				@battle.pbDisplay(_INTL("{1} blew away the sticky web!",attacker.pbThis))     
			end
...
# already moved.
################################################################################
class PokeBattle_Move_116 < PokeBattle_Move#PokeBattle_UnimplementedMove
	def pbMoveFailed(attacker,opponent)
