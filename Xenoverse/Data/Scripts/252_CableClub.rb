...
def pbCableClub
  if !$MKXP
    Kernel.pbMessage(_INTL("The new engine is required to play online."))
    return
  end
...
module CableClub
  ...
  if (!defined?(ESSENTIALSVERSION) && !defined?(ESSENTIALS_VERSION)) || (defined?(ESSENTIALS_VERSION) && ESSENTIALS_VERSION > 15)
      ...
      for index in CableClub::pokemon_order(@client_id)
        ...
        if !pbOwnedByPlayer?(index)
          if !pbIsOpposing?(index) || (@opponent && pbIsOpposing?(index))
            ...
            pbRecallAndReplace(index,newenemy,false)
            switched.push(index)
          end
        else
