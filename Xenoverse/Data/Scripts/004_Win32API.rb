if !$MKXP
  ...
  def Win32API.pbFindRgssWindow
    return @@RGSSWINDOW if @@RGSSWINDOW
    processid=[0].pack('l')
    threadid=Win32API.new('kernel32','GetCurrentThreadId', '%w()','l').call
    nextwindow=0
    begin
      nextwindow=Win32API.new('user32','FindWindowEx', '%w(l l p p)','l').call(0,nextwindow,"Game.exe",0)
      if nextwindow!=0
        wndthreadid=Win32API.new('user32','GetWindowThreadProcessId', '%w(l p)','l').call(nextwindow,processid)
        if wndthreadid==threadid
          @@RGSSWINDOW=nextwindow
          return @@RGSSWINDOW 
        end
