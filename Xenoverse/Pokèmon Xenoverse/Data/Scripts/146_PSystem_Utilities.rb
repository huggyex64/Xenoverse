...
def pbPokemonBitmapFileForm(species, form, shiny, back=false)
	if shiny
		# Load shiny bitmap
		ret=sprintf("Graphics/Battlers/Front/%ss%s_#{form}",getConstantName(PBSpecies,species),back ? "b" : "") rescue nil
		if !pbResolveBitmap(ret)
			ret=sprintf("Graphics/Battlers/Front/%03ds%s_#{form}",species,back ? "b" : "")
		end
		return ret
	else
		# Load normal bitmap
		ret=sprintf("Graphics/Battlers/Front/%s%s_#{form}",getConstantName(PBSpecies,species),back ? "b" : "") rescue nil
		if !pbResolveBitmap(ret)
			ret=sprintf("Graphics/Battlers/Front/%03d%s_#{form}",species,back ? "b" : "")
		end
		return ret
	end
end
...
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false)
	...
	if pokemon.isEgg?
		...
		bitmapFileName=pbCheckPokemonBitmapFiles([species,back,
				...
				(pokemon.isShadow? rescue false),(pokemon.busted rescue false)])
...
def pbCheckPokemonBitmapFiles(params)
	...
	factors.push([6,params[6],false]) if params[6] && params[6]!=false     # busted (for mimikyu)
...
def pbPlayCry(pokemon,volume=100,pitch=nil)
	return if !pokemon
	if pokemon.is_a?(Numeric)
...
def pbHasStarter?
	ret = false
	for i in 0...$Trainer.party.length
		if $Trainer.party[i].species == PBSpecies::TRISHOUT || $Trainer.party[i].species == PBSpecies::SHULONG || $Trainer.party[i].species == PBSpecies::SHYLEON
			ret=true
		end
	end
	return ret
...
def pbHasSabolt?
	found = false
	for i in 0...$Trainer.party.length
		if $Trainer.party[i].species == PBSpecies::SABOLT
			found = true
			return true
		else
			next
...
def pbHasTrishout?
	found = false
	for i in 0...$Trainer.party.length
		if $Trainer.party[i].species == PBSpecies::TRISHOUT
			found = true
			return true
		else
			next
...
def pbHasShyleon?
	found = false
	for i in 0...$Trainer.party.length
		if $Trainer.party[i].species == PBSpecies::SHYLEON
			found = true
			return true
		else
			next
...
def pbHasShulong?
	found = false
	for i in 0...$Trainer.party.length
		if $Trainer.party[i].species == PBSpecies::SHULONG
			found = true
			return true
		else
			next
...
def pbTakeStarter
	for i in 0...$Trainer.party.length
		return false if i<0 || !$Trainer || i>=$Trainer.party.length
		if i==0
			if $Trainer.party[0].species == PBSpecies::TRISHOUT || $Trainer.party[0].species == PBSpecies::SHULONG || $Trainer.party[0].species == PBSpecies::SHYLEON
				$starter=$Trainer.party[0]
				$Trainer.party.delete_at(0)
			end
		else
			if $Trainer.party[i].species == PBSpecies::TRISHOUT || $Trainer.party[i].species == PBSpecies::SHULONG || $Trainer.party[i].species == PBSpecies::SHYLEON
				$starter=$Trainer.party[i]
...
def pbTakeMagikarp
	for i in 0...$Trainer.party.length
		return false if i<0 || !$Trainer || i>=$Trainer.party.length
		if i==0
			if $Trainer.party[0].species == PBSpecies::MAGIKARP
				$Trainer.party.delete_at(0)
			end
		else
			if $Trainer.party[i].species == PBSpecies::MAGIKARP
				$Trainer.party.delete_at(i)      
			end
		end
	end
...
def pbTakeScaleon
	for i in 0...$Trainer.party.length
		return false if i<0 || !$Trainer || i>=$Trainer.party.length
		if i==0
			if $Trainer.party[0].species == PBSpecies::SCALEONOLEANDRO
				$Trainer.party.delete_at(0)
			end
		else
...
def pbGiveStarter
	if $starter != nil
		$Trainer.party.unshift($starter)
		$starter=nil
	else
		Kernel.pbMessage("No stored starters.")
	end
end

#==================================
# pbSetStatus
#   status ===> "sleep","poison","burn","paralyzed","frozen" con le ""
#   allparty ===> true o false, applica l'effetto a tutto il party o a un solo pokemon
#   index ===> NECESSARIO se allparty=false, numero da 0 a 5 che indica il pokèmon nella squadra su cui applicare lo status

def pbSetStatus(status,allparty=true,index=nil)
	party = $Trainer.party
	case status
	when "sleep"
		if allparty==true
...
def pbMoveTutorAnnotations(move,movelist=nil)
	...
	for i in 0...6
		...
		if !$Trainer.party[i].isEgg? && movelist && movelist.any?{|j| j==species }
			...
			ret[i]=_INTL("ABLE")
		elsif !$Trainer.party[i].isEgg?
			if $Trainer.party[i].species==PBSpecies::BREMAND
				if pbCheckBremandCompat($Trainer.party[i],move) 
					ret[i]=_INTL("ABLE")
				else
					ret[i]=_INTL("NOT ABLE")
				end
			else
				if pbSpeciesCompatible?(species,move)
...
BREMANDTMS=[[:ROAR,:TOXIC,:HIDDENPOWER,:TAUNT,:PROTECT,:ROOST,
		:SUBWOOFER,:RETURN,:SHADOWBALL,:DOUBLETEAM,:AERIALACE,
		:REST,:ATTRACT,:ROUND,:ECHOEDVOICE,:FLING,:SHADOWCLAW,:SUBSTITUTE],
		[:HONECLAWS,:ROAR,:TOXIC,:HIDDENPOWER,:TAUNT,:PROTECT,
		:SUBWOOFER,:THUNDERBOLT,:THUNDER,:RETURN,:DOUBLETEAM,
		:REST,:ATTRACT,:ROUND,:ECHOEDVOICE,:FLING,:CHARGEBEAM,
		:VOLTSWITCH,:THUNDERWAVE,:SUBSTITUTE,:WILDCHARGE],
		[:ROAR,:TOXIC,:BULKUP,:HIDDENPOWER,:TAUNT,:PROTECT,:SUBWOOFER,
		:RETURN,:BRICKBREAK,:DOUBLETEAM,:ROCKTOMB,:REST,:ATTRACT,:LOWSWEEP,
		:ROUND,:ECHOEDVOICE,:FOCUSBLAST,:FLING,:POWERUPPUNCH,:SUBSTITUTE],
	  [:ROAR,:TOXIC,:HIDDENPOWER,:TAUNT,:PROTECT,:SUBWOOFER,
		:RETURN,:SOUBLETEAM,:ROCKTOMB,:TORMENT,:REST,:ATTRACT,
		:THIEF,:ROUND,:ECHOEDVOICE,:FLING,:BRUTALSWING,:PAYBACK,
		:SUBSTITUTE,:DARKPULSE],
		
	]

def pbCheckBremandCompat(pokemon,move)
	echoln "pokemon form is #{pokemon.form}"
	return false if pokemon.form>=BREMANDTMS.length
	for m in BREMANDTMS[pokemon.form]
		#mv=getID(PBMoves,m)
		#echoln "move is #{mv}, checking #{move}"
		if isConst?(move,PBMoves,m)
			echoln "move is learnable"
			return true 
		end
	end
	return false
...
def pbMoveTutorChoose(move,movelist=nil,bymachine=false)
	...
	pbFadeOutIn(99999){
		...
		screen=PokemonScreen.new(scene,$Trainer.party)
		annot=pbMoveTutorAnnotations(move,movelist) #HERE FOR BREMAND COMPATIBILITY
		...
		loop do
			...
			if chosen>=0
				...
				elsif pokemon.species == PBSpecies::BREMAND
					if pbCheckBremandCompat(pokemon,move)
				...
				elsif !pbSpeciesCompatible?(pokemon.species,move) #HERE FOR BREMAND COMPATIBILITY
					Kernel.pbMessage(_INTL("{1} is not compatible with {2}.",pokemon.name,movename))
					Kernel.pbMessage(_INTL("{1} can't be learned.",movename))
				else
					if pbLearnMove(pokemon,move,false,bymachine)
