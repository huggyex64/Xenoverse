...
def pbHiddenMoveHoloAnimation(path,pokemon)
  return false if !pokemon
  viewport=Viewport.new(0,0,0,0)
  viewport.z=99999
  bg=Sprite.new(viewport)
  bg.bitmap=BitmapCache.load_bitmap("Graphics/Pictures/hiddenMovebg")
  sprite=BitmapWrapperSprite.new(viewport)
  sprite.setBitmap(path+"_holo")
  sprite.z=1
  sprite.ox=sprite.bitmap.width/2
  sprite.oy=sprite.bitmap.height/2
  sprite.visible=false
...
HiddenMoveHandlers::CanUseMove.add(:CUT,proc{|move,pkmn|
   ...
   if !facingEvent || facingEvent.name!="Tree"
     Kernel.pbMessage(_INTL("Non puoi usarla ora."))
     return false
   end
   return true
})
...
HiddenMoveHandlers::UseMove.add(:CUT,proc{|move,pokemon|
   if !pbHiddenMoveHoloAnimation("Graphics/Battlers/Front/123",pokemon)
     Kernel.pbMessage(_INTL("{1} usa {2}!",pokemon.name,PBMoves.getName(move)))
   end
   facingEvent=$game_player.pbFacingEvent
   if facingEvent
     facingEvent.erase
...
HiddenMoveHandlers::UseMove.add(:ROCKSMASH,proc{|move,pokemon|
   if !pbHiddenMoveHoloAnimation("Graphics/Battlers/Front/328",pokemon)
     Kernel.pbMessage(_INTL("Holo-Trapinch usa {2}!",pokemon.name,PBMoves.getName(move)))
   end
   facingEvent=$game_player.pbFacingEvent
   if facingEvent
     facingEvent.erase
...
HiddenMoveHandlers::UseMove.add(:STRENGTH,proc{|move,pokemon|
   pbHiddenMoveHoloAnimation("Graphics/Battlers/Front/1046",pokemon)
   Kernel.pbMessage(_INTL("Holo-Hercurcules usa {2}!\1",pokemon.name,PBMoves.getName(move)))
   Kernel.pbMessage(_INTL("La forza di Holo-Hercurcules ti permette di spostare i massi!",pokemon.name))
   $PokemonMap.strengthUsed=true
   return true  
})
...
def Kernel.pbLavaSurf
  if $game_switches[BADGEFORLAVASURFING]
    if Kernel.pbConfirmMessage(_INTL("The lava is a deep red...\nWould you like to surf on it?"))
      Kernel.pbMessage(_INTL("Torkoal ti aiuta ad attraversare"))
    ...
    Kernel.pbMessage(_INTL("Un Pokémon potrebbe aiutarti ad attraversare\nla lava..."))
  end
  return false
end
...
HiddenMoveHandlers::CanUseMove.add(:TELEPORT,proc{|move,pkmn|
   Kernel.pbMessage(_INTL("A causa di una distorsione dimensionale, il Pokémon non può usare Teletrasporto."))
=begin
   if pbGetMetadata($game_map.map_id,MetadataOutdoor)
     Kernel.pbMessage(_INTL("Can't use that here."))
     return false
