...
module Scripts
  def self.dump(path = "Data/Scripts", rxdata = "Data/Scripts.xvd")
    if (!File.exists?("Data/Scripts.xvd"))
        File.open(rxdata,'w'){}
    end
...
#    p loader_scripts
  ...
  def self.from_folder(path = "Data/Scripts", rxdata = "Data/Scripts.xvd")
    if (!File.exist?(rxdata))
  ...
  def self.aggregate_from_folder(path, scripts, level = 0)
    ...
    files.each do |f|
      ...
      scripts << [rand(999_999), section_name, Zlib::Deflate.deflate(Base64.encode64(content))]
    end
  ...
  def self.aggregate_from_folder_std(path,scripts,level)
    files = []
    folders = []
    Dir.foreach(path) do |f|
      next if f == '.' || f == '..'
...
#Scripts.dump
Scripts.from_folder("Data/Scripts")